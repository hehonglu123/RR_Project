...
class Planner(object):
	def __init__(self):
		...
		with open('calibration/Staubli.yaml') as file:
			H_Staubli 	= np.array(yaml.load(file)['H'],dtype=np.float64)
		self.H_Sawyer=H42H3(H_Sawyer)
		self.H_ABB=H42H3(H_ABB)
		self.H_Staubli=H42H3(H_Staubli)
		self.H_UR=H42H3(H_UR)

		self.transformation={'sawyer':H_Sawyer,'abb':H_ABB,'ur':H_UR,'staubli':H_Staubli}
		...
		Staubli_joint_names=['Staubli_joint_1','Staubli_joint_2','Staubli_joint_3','Staubli_joint_4','Staubli_joint_5','Staubli_joint_6']
		Staubli_link_names=['Staubli_base_link','Staubli_link_1','Staubli_link_2','Staubli_link_3','Staubli_link_4','Staubli_link_5','Staubli_link_6']

		#Robot dictionaries, all reference by name
		self.robot_name_list=['sawyer','abb','ur','staubli']
		self.robot_linkname={'sawyer':Sawyer_link_names,'abb':ABB_link_names,'ur':UR_link_names,'staubli':Staubli_link_names}
		self.robot_jointname={'sawyer':Sawyer_joint_names,'abb':ABB_joint_names,'ur':UR_joint_names,'staubli':Staubli_joint_names}
		...
		cmd3 = ChangeJointOriginCommand("ur_pose", Isometry3d(H_UR))
		cmd4 = ChangeJointOriginCommand("staubli_pose", Isometry3d(H_Staubli))

		self.t_env.applyCommand(cmd1)
		self.t_env.applyCommand(cmd2)
		self.t_env.applyCommand(cmd3)
		self.t_env.applyCommand(cmd4)
		...
		self.robot_N_step={'sawyer':np.zeros((self.N_step+1,7)),'abb':np.zeros((self.N_step+1,6)),'ur':np.zeros((self.N_step+1,6)),'staubli':np.zeros((self.N_step+1,6))} #0 to N_step
		self.u_all={'sawyer':np.zeros(self.N_step*7),'abb':np.zeros(self.N_step*6),'ur':np.zeros(self.N_step*6),'staubli':np.zeros(self.N_step*6)} #0 to N_step-1 control input qdot
