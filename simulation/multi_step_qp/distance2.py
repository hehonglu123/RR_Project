...
class create_impl(object):
	...
	def distance_check(self,robot_idx,joints,dt):
		with self._lock:
			...
			robot_joints+=0.2*np.cos(time.time()*np.ones(len(robot_joints),))-0.2*np.cos((time.time()+dt)*np.ones(len(robot_joints),))
...
with RR.ServerNodeSetup("Distance_Service", 25522) as node_setup:
	...
	RRN.RegisterServiceTypeFromFile("edu.rpi.robotics.distance")
