	...
	while True:
		...
		for i in range(len(entry.elements)):
			...
			if el.ElementName=='packettime':
				...
				elements = el.GetData().Elements
				seconds=reader.UnpackMessageElement(elements[0]).data[0]
				nanoseconds=reader.UnpackMessageElement(elements[1]).data[0]
				temp_el[el.ElementName]=seconds+nanoseconds*1e-9
